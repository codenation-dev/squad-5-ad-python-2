...
class EmailListComission(APIView):
    ...
    def post(self, request):
        ...
        comissions = [s.comission for s in sales]
        if len(comissions)>=5:
            comissions = np.array(sorted(comissions[-5:]))
            weights = np.array([0,0,1,2,3])
            result = (comissions.dot(weights)/weights.sum())*.9
            if (request.data['amount']<= result):
                subject = 'Notificação'
            ...
            res = {"seller": seller.id, "result": result}
            return Response(res,status=status.HTTP_200_OK)
        else:
